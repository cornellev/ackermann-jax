...
Subscriptions (measurements):
    ...
    /wheel_speeds     std_msgs/Float32MultiArray   -- wheel angular velocities [FL,FR,RL,RR] rad/s
...
def h_wheels(x: AckermannCarState) -> jnp.ndarray:
    """Wheel encoders: measures wheel angular velocities [FL,FR,RL,RR]."""
    return x.omega_W
...
class EKFNode(Node):
    def __init__(self):
        ...
        self._R_wheels   = r_wheels  * jnp.eye(4)
    ...
    def _cb_imu(self, msg: Imu) -> None:
        ...
        self._z_gyro = jnp.array(
            [av.x, av.y, av.z], dtype=jnp.float32
        )

        la = msg.linear_acceleration
        self._z_gravity = jnp.array(
            [la.x, la.y, la.z], dtype=jnp.float32
        )

    def _cb_wheels(self, msg: Float32MultiArray) -> None:
        if len(msg.data) < 4:
            self.get_logger().warn(
                f"Expected 4 wheel speeds, got {len(msg.data)}"
            )
            return
        self._z_wheels = jnp.array(msg.data[:4], dtype=jnp.float32)
    ...
    def _try_init(self, gx: float, gy: float) -> None:
        ...
        omega0 = (
            ...
            else jnp.zeros(4, dtype=jnp.float32)
        )
    ...
    def _timer_cb(self) -> None:
        ...
        if self._z_gps is not None:
            self._ekf = ekf_update(
                self._ekf, self._z_gps, h_gps_2d, self._R_gps
            )

        # ── Update: gyroscope ──
        if self._z_gyro is not None:
            self._ekf = ekf_update(
                self._ekf, self._z_gyro, h_gyro, self._R_gyro
            )
        ...
        if self._z_wheels is not None:
            self._ekf = ekf_update(
                self._ekf, self._z_wheels, h_wheels, self._R_wheels
            )
